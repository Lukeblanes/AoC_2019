...
coordinate check_teleportation(coordinate *new, int32_t* level_offset)
{
...
uint32_t shortest_path(coordinate *previous, coordinate *current, uint32_t steps, int32_t level)
{
    uint32_t minimum = MAX_STEPS;
    if(steps >= MAX_STEPS) {
        return steps;
    } else if(level == 0 && current->x == END_POS.x && current->y == END_POS.y) {
        return steps;
    } else {
        coordinate newPos;
        coordinate teleportation_check;
        uint32_t aux = MAX_STEPS;
        ...
        for (uint32_t i = 0; i < 4; i++)
        {
            ...
            {
                teleportation_check = check_teleportation(&newPos, &level_offset);
                if(teleportation_check.x == -1 && teleportation_check.y == -1) { // no teleporting
                    aux = shortest_path(current, &newPos, steps + 1, level);
                } else { // teleporting so previous is now other side of portal and new is the next position from portal 
                    if(level + level_offset >= 0) // if we can explore an outer level we go 
                    {
                        newPos = next_position_from_portal(&teleportation_check);
                        aux = shortest_path(&teleportation_check, &newPos, steps + 3, level + level_offset);
                    }
                }
...
int main(int argc, char** argv)
{
    ...
    uint32_t result = shortest_path(&START_POS, &((coordinate) {.x = START_POS.x + INITIAL_DIR.x, .y = START_POS.y + INITIAL_DIR.y}), 1, 0); // First move will always be in one direction
    // Improvable takes 27 seconds with -O3 optimization
