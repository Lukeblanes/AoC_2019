...
uint32_t get_longest_path(char* grid, uint32_t grid_width, coordinate previous, coordinate new)
{
    coordinate aux;
    uint32_t max_aux, max = 0;
    for (uint32_t i = 0; i < 4; i++)
    {
        ...
        if((!(aux.x == previous.x && aux.y == previous.y)) && grid[aux.y * grid_width + aux.x] == '.')
        {
            ...
            if(paths != 0)
                max_aux = 1 + get_longest_path(grid, grid_width, new, aux);
            else
                max_aux = 1;
        }
        if(max_aux > max)
...
int main(int argc, char** argv)
{
    ...
    while (g_end_program == 0 && iterations++ <= 100000)
    {
        ...
        // iterations in case in the first iterations it returns to original pos
...
//    print_grid(grid, grid_width, smallest_x, biggest_x);
    ...
    coordinate first_next_step = { oxygen.x + 1, oxygen.y};
    uint32_t result = 1 + get_longest_path(grid, grid_width, oxygen, first_next_step);
 
    printf("Oxygen would take to fill up %d minutes.\n", result); // output -> 390
