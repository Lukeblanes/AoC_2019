...
{
    // length * length grid
    for (uint32_t i = 1; i < length * 6 + 1; i++)
    {
        if(i > length * 6 + 1) // if we print six lines, we exit
            break;

        printf("%c", grid[i-1]);
        if (i % length == 0 ) // printline 6 rows for output
        {
            printf("\n");
        }
    }
...
int main(int argc, char** argv)
{
    ...
    uint32_t grid_width = 45; // Hardcoded value to print only the message
    ...
    coordinate robot_pos = {0, 0};  // 0, 0: since i know robot won't go up or left

    while (g_end_program == 0)
    {
        char current_char = grid[grid_width * robot_pos.y + robot_pos.x];

        if(current_char == '#' || current_char == '-') // Consider first tile white
            g_program_input = 1; 
        else
            g_program_input = 0;
    ...
    print_grid(grid, grid_width); // Output -> CEPKZJCR
